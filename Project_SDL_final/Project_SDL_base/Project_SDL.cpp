...
{
    this->window_ptr_ = SDL_CreateWindow("Barn Simulation", SDL_WINDOWPOS_UNDEFINED,
...
int application::loop(unsigned int period)
{
    ...
    while (!close && elapsed_time < period * 1000)
    {
        ...
        SDL_FillRect(this->window_surface_ptr_, NULL,
                     SDL_MapRGB(this->window_surface_ptr_->format, 21, 71, 52));


        bool end = this->my_ground->update();
        if (end)
        {
            period = 0;
        }
